...
def analyze_car(car_data):
    try:
        ...
        ТРЕБОВАНИЯ К ОТЧЕТУ:
        1. Будь ОБЪЕКТИВЕН - указывай реальные проблемы, а не общие фразы
        2. Используй данные с Drive2.ru, Drom.ru, отзывы реальных владельцев
        3. Указывай КОНКРЕТНЫЕ пробеги, когда проявляются проблемы
        4. Цены на ремонт указывай в рублях для России 2024
        5. Если не уверен в данных - пиши "требует проверки"

        СТРУКТУРА ОТЧЕТА:
        ⚠️ КРИТИЧЕСКИЕ ПРОБЛЕМЫ (самые дорогие и частые поломки)
        🔧 ДВИГАТЕЛЬ (ресурс, типичные неисправности, стоимость ремонта)
        ⚙️ КОРОБКА ПЕРЕДАЧ (надежность, проблемы, обслуживание)
        🛞 ПОДВЕСКА (слабые места, ресурс элементов)
        ⚡ ЭЛЕКТРИКА (частые сбои, проблемы с электроникой)
        📋 ЧТО ПРОВЕРИТЬ (конкретный чек-лист для осмотра)

        НЕ ПРИУКРАШИВАЙ реальность! Владельцы должны знать все риски.
        """
        ...
        data = {
            ...
            "temperature": 0.3,  # Снижаем "творческость"
            "max_tokens": 2500
        }
